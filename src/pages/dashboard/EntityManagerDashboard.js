              ...
              <CardContent>
                ...
                <Box sx={{ mt: 2, display: 'flex', gap: 1 }}>
                  <Button
                    ...
                    sx={{ flex: 1 }}
                  >
                    {selectedEntity === entity.id ? "Selected" : "Select"}
                  </Button>
                  <Button
                    variant="contained"
                    color="primary"
                    size="small"
                    onClick={() => {
                      // Map the entity IDs to the actual entity IDs from ComplianceContext
                      const entityIdMap = {
                        'entity-1': 'ent-001', // ABC Private Limited
                        'entity-2': 'ent-002', // XYZ Solutions -> maps to ABC Enterprises LLP
                        'entity-3': 'ent-001'  // DEF Industries -> fallback to ent-001
                      };
                      const actualEntityId = entityIdMap[entity.id] || 'ent-001';
                      navigate(`/entities/${actualEntityId}/details`);
                    }}
                    sx={{ flex: 1 }}
                  >
                    View Details
